...
// template<typename T>
// struct is_printable<T, decltype(std::is_base_of_v<Printable<T>, T>)> : std::true_type {};
...
constexpr auto& print(const std::string_view& str)
{
    return print<std::string_view>(str);
}

constexpr auto& print(const std::string& str)
{
